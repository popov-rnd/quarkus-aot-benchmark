...
In Quarkus, an über-jar is simply a single runnable JAR that contains:

- Your application classes
- All dependencies (third-party JARs) merged inside
- Quarkus’s generated bytecode from its build-time augmentation

It’s the "one file to run them all" packaging option.
...
Run both builds (über-jar and native)

📊 Measure:

- Build time → taken from Maven’s own build output (e.g., [INFO] BUILD SUCCESS in _ s).
- Startup time → taken from Quarkus’s startup log (e.g., Quarkus started in _ s).
